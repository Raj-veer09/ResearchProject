...
# Optional quarantine/response module — integrated if present in the same folder.
# If quarantine_response.py is missing, the pipeline still runs (quarantine skipped).
try:
    from quarantine_response import handle_response as _quarantine_handle_response
...
load_dotenv()
API_KEY = os.getenv("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")
client = Anthropic(api_key=API_KEY) if API_KEY else None


def extract_json(text):
    """
    Robustly extract the classification JSON from Claude's response, even when
    the reasoning text contains stray braces (Base64 payloads, JSON examples,
    code snippets). Tries multiple strategies and returns the LAST valid object
    that has a 'classification' key. Returns None if nothing parseable found.
    """
    if not text:
        return None
    text = text.strip()

    # Strategy 0 — fenced ```json ... ``` block
    ...
    # Strategy 1 — whole text (after stripping stray fences) is JSON
    ...
    # Strategy 2 — brace-count every top-level {...} object, take LAST valid one
...
# ── Local signature database (hash blocklist) ──────────────────────────────
HASH_BLOCKLIST_FILE = "malicious_hashes.txt"


def load_known_bad_hashes():
    """Read confirmed-malicious hashes from the blocklist file into a set."""
    known = set()
    if os.path.exists(HASH_BLOCKLIST_FILE):
        with open(HASH_BLOCKLIST_FILE, "r") as f:
            for line in f:
                line = line.strip()
...
def store_malicious_hash(file_hash, process_name, reason):
    """
    Append a confirmed-malicious file hash to malicious_hashes.txt.
    This grows into a local signature database. Skips 'unknown' and duplicates.
    """
    if not file_hash or file_hash == "unknown":
        return False
    if file_hash in load_known_bad_hashes():
        return False
    new_file = not os.path.exists(HASH_BLOCKLIST_FILE)
    with open(HASH_BLOCKLIST_FILE, "a") as f:
        if new_file:
            f.write("# IEPIS Malicious Hash Blocklist (local signature DB)\n")
            f.write("# Format: SHA256_HASH  process_name  timestamp  reason\n")
        f.write(f"{file_hash}  {process_name}  {now_iso()}  {str(reason).replace(chr(10), ' ')[:150]}\n")
    return True
...
def get_process_detail(pid: int) -> dict | None:
    try:
        ...
        return {
            ...
            "Internal_Process_Executable_Path": exe_path or None,
        }
...
def query_llm(prompt, max_retries=5):
    ...
    for attempt in range(max_retries):
        try:
            ...
            # Collect ALL text blocks (web search produces several) — not just the last
            ...
            # Robust JSON extraction (handles stray braces in reasoning)
            result = extract_json(final_text)
            if result is None:
                print(f"    [WARN] No parseable JSON. Raw tail: {final_text[-200:]!r}")
                raise json.JSONDecodeError("No JSON object found", final_text, 0)

            # Preserve the reasoning text (everything before the JSON fence) for the log
            result["threat_intel_text"] = re.split(r"```(?:json)?", final_text)[0].strip()
            return result
...
def process_and_log_event(event_type, combined_data, args):
    ...
    proc_data = {
        ...
        "Internal_Process_Executable_Path": combined_data.get("Internal_Process_Executable_Path")
    }
    ...
    # Full reasoning text for the log (falls back to the short finding)
    ti_text = result.get("threat_intel_text", "") or row["AI__Threat_Intel_Finding"]

    # ── Store hash if MALICIOUS (local signature DB for Windows Defender) ──
    if row["AI__Model_classification"] == "MALICIOUS":
        if store_malicious_hash(row.get("process_hash_sha256"),
                                row.get("process_name"),
                                row["AI__Model_Reason"]):
            print(f"  [HASH STORED] {str(row.get('process_hash_sha256'))[:32]}... -> malicious_hashes.txt")

    # ── Quarantine / Alert response (Layer 4 file defense) ──
    # HIGH-confidence MALICIOUS -> quarantine the file; LOW/MEDIUM -> alert only.
    if QUARANTINE_AVAILABLE and not getattr(args, "no_quarantine", False):
        try:
            _quarantine_handle_response(row, result)
        except Exception as e:
            print(f"  [QUARANTINE] skipped due to error: {e}")
    ...
    with open(args.out_log, "a", encoding="utf-8") as f:
        ...
        f.write(f"Threat Intel: {ti_text}\n")
...
def main():
    ...
    parser.add_argument("--no-quarantine", action="store_true", help="Disable the quarantine/alert response layer.")
