...
def get_process_detail(pid: int) -> dict | None:
    """
    Full attribute extraction for a SINGLE pid (hash + cert check).
    Called only for newly-detected processes, not the whole baseline.
    """
    try:
...
def scan_connections() -> dict[tuple, dict]:
    ...
    conn_key = (pid, local_ip, local_port, remote_ip, remote_port, status)
    -- a tuple key so the SAME connection (not just same PID) is tracked.
    """
    current = {}
    try:
...
def main():
    ...
    # ── Initial baseline scan (FAST — no hash/cert checks) ──
    known_processes = scan_processes_light()
    known_connections = {}  # start empty; first cycle will populate without emitting
    ...
    try:
        while True:
            ...
            for key in new_conn_keys:
                ...
                # Attach process context if we know this PID (the join) — light lookup only
